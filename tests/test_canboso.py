...
async def test_canboso_lists_products_with_query_key_and_parses_special_requirements() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert request.url.path == "/api/v2/telegram-buyer/products"
                ...
                "products": [
                    {
                        ...
                        "productType": "account",
                        "price": {"amount": 1.85, "currency": "USD", "text": "$1.85"},
                        "availability": {"sold": 40, "available": 60},
                        "promotions": [],
                    },
                    {
                        ...
                        "availability": {"available": 12, "sold": 8},
...
@pytest.mark.asyncio
async def test_canboso_v2_generic_slot_does_not_request_months_and_allows_unknown_stock() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/api/v2/telegram-buyer/products"
        return httpx.Response(
            200,
            json={
                ...
                "products": [
                    {
                        "productId": "youtube-slot-1m",
                        "name": "YouTube Premium Slot 1M",
                        "productType": "slot",
                        "price": {"amount": 1.7, "currency": "USD", "text": "$1.70"},
                        "availability": {"available": None, "sold": 25},
                        "purchaseRequirements": {
                            "customerEmail": True,
                            "slotMonths": False,
                            "quantityFixed": 1,
    ...
    assert product.requires_customer_email is True
    assert product.requires_slot_months is False
    assert product.slot_durations == ()
    assert product.quantity_fixed == 1
...
async def test_canboso_balance_preserves_native_text_and_usdt_equivalent() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/api/v2/telegram-buyer/balance"
        assert request.url.params["key"] == "tgb_test_key"
        return httpx.Response(
            200,
            json={
...
async def test_canboso_purchase_sends_key_and_special_fields_and_parses_delivery() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert request.url.path == "/api/v2/telegram-buyer/purchase"
    ...
    assert "2026-09-24" in order.delivery_payload
